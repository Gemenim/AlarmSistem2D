using System.Collections;
using System.Collections.Generic;
using UnityEngine;

[RequireComponent(typeof(AudioSource))]
[RequireComponent(typeof(IntruderLocator))]
public class Annunciator : MonoBehaviour
{
    [SerializeField] private float _rateÑhange;

    private AudioSource _audioSource;
    private IntruderLocator _intruderLocator;
    private float _maxVolume = 1.0f;
    private bool _isCorutinJob = false;

    public void StartChangeVolume()
    {
        _isCorutinJob = true;
        StartCoroutine(ChangeVolume());
    }

    private void Start()
    {
        _audioSource = GetComponent<AudioSource>();
        _intruderLocator = GetComponent<IntruderLocator>();
        _audioSource.volume = 0;
    }

    private IEnumerator ChangeVolume()
    {
        while (_isCorutinJob)
        {
            if (_intruderLocator.IsIntruder)
                _audioSource.volume += _maxVolume * Time.deltaTime * _rateÑhange;
            else
            {
                _audioSource.volume -= _maxVolume * _rateÑhange * Time.deltaTime;

                if (_audioSource.volume == 0)
                    _isCorutinJob = false;
            }

            yield return new WaitForEndOfFrame();
        }
    }
}
